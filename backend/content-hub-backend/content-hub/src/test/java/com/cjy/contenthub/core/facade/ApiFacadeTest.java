...
class ApiFacadeTest {
	...
	void test_initializeTMdbApiGenreInfo() {
		...
		when(tmdbGenreService.getTvGenres()).thenReturn(CompletableFuture.completedFuture(tvGenreMap));
		when(tmdbGenreService.getMovieGenres()).thenReturn(CompletableFuture.completedFuture(movieGenreMap));
	...
	void test_getTvGenres() {
		...
		when(tmdbGenreService.getTvGenres()).thenReturn(CompletableFuture.completedFuture(genreMap));
	...
	void test_getMovieGenres() {
		...
		when(tmdbGenreService.getMovieGenres()).thenReturn(CompletableFuture.completedFuture(genreMap));
	...
	void test_getTranslationText() throws InterruptedException, ExecutionException, TimeoutException {
		...
		// 실제 메서드 호출
		String result = apiFacade.getTranslationText(text, targetLang, sourceLang).get(5, TimeUnit.SECONDS);
