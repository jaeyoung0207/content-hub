...
/**
 * 어플리케이션 컨트롤러 클래스
 * 어플리케이션 전반에서 공통으로 쓰이는 API
 */
@Tag(name = "app-api", description = "App APIs")
@ApiController
@RequestMapping("/app")
...
public class AppController {
    ...
    public AppMediaTypeResponseDto getMediaTypes() {
    	// 컨텐츠 미디어 타입 DTO 생성
    	AppContentMediaTypeDto contentMediaTypeDto = AppContentMediaTypeDto.builder()
    			...
    			.build();
    	// 화면표시용 미디어 타입 DTO 생성
    	AppDisplayMediaTypeDto displayMediaTypeDto = AppDisplayMediaTypeDto.builder()
		...
		return AppMediaTypeResponseDto.builder()
	...
	public ResponseEntity<Void> setAdultFlg(@RequestParam(DomainConstants.ADULT_FLG) boolean adultFlg) {
		
		// 세션을 가져옴
		HttpSession session = sessionUtil.getSession();
		// 세션에서 성인 여부 플래그를 설정
		session.setAttribute(DomainConstants.ADULT_FLG, adultFlg);
	...
	public ResponseEntity<Void> clearAdultFlg() {
		...
		if (ObjectUtils.isNotEmpty(session.getAttribute(DomainConstants.ADULT_FLG))) {			
			// 세션에서 성인 여부 플래그를 제거
			session.removeAttribute(DomainConstants.ADULT_FLG);
		}
