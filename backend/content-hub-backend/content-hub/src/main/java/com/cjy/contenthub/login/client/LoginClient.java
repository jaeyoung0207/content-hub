...
public class LoginClient {
	 ...
	 */
	public CompletableFuture<ResponseEntity<LoginUserResponseDto>> getNaverUserInfo(
		...
		return naverWebClient.get()
				...
				.toFuture()
				// 비동기 처리 및  Blocking 작업을 Executor에 위임
				.thenComposeAsync(response -> {
					...
					// user 등록 확인 후 등록(비동기 처리 및 Blocking 작업을 한번 더 Executor에 위임)
					return CompletableFuture.supplyAsync(() -> service.saveUser(userServiceDto), blockingTaskExecutor)
							.thenApply(saveResponse -> {
								// JWT 및 만료일자 생성
								...
								if (StringUtils.isNotEmpty(refreshToken)) {
									// 로그인 쿠키 설정
									String[] cookieArray = loginHelper.setLoginCookies(request, refreshToken, provider, providerId, expiresIn);
	 ...
	 * @return 유저 정보 응답 오브젝트
	 */
	public CompletableFuture<ResponseEntity<LoginUserResponseDto>> getKakaoUserInfo(
			HttpServletRequest request, 
			String accessToken,
			int expiresIn,
			String refreshToken
