...
@Subselect(
		"SELECT * FROM ("
				+ "SELECT ROW_NUMBER() OVER (PARTITION BY con.original_media_type ORDER BY AVG(com.star_rating) DESC, COUNT(com.star_rating) DESC) as row_num, "
				+ "con.content_id , con.original_media_type , con.api_id , AVG(com.star_rating) AS star_rating_average, "
				...
				+ "GROUP BY con.content_id, con.original_media_type , con.api_id, con.title, con.thumbnail_image_url"
				+ ") top "
				+ "WHERE top.row_num <= 10 "
				+ "ORDER BY top.original_media_type, top.star_rating_average DESC, top.star_rating_count DESC"
		)
