 ...
 */
export const SearchTextUi = memo(<T extends FieldValues>({ name, control, onClick, onMouseDown, onKeyDown, isFocusedRef, deleteValue }: FormFieldProps<T>) => {
    // i18n 번역 훅
        ...
        <>
            <div className="flex lg:w-sm w-xs lg:h-10 items-center border rounded-md shadow-sm">
                <Controller
                    name={name}
                    control={control}
                    render={({ field: { value, onBlur, onChange, ref } }) => { // react-hook-form이 제공하는 필드 컨트롤
                        return (
                            <>
                                <input
                                    type="text"
                                    ...
                                    // onBlur={onBlur} // touched 상태(입력값 변경 여부) 추적 및 유효성 검증 (mode: 'onBlur' 대응)
                                    onBlur={() => {
                                ...
                                />
                                {
                                    ...
                                    <div className="mr-1 flex justify-center items-center">
                                        <button className="w-5 h-5 text-sm text-gray-400 cursor-pointer" onClick={deleteValue && commonErrorHandler(deleteValue)}>X</button>
                                    </div>
                                }
                                {/* 검색 아이콘 */}
                                <BiSearch className="mr-3 w-6 h-6 cursor-pointer" onClick={value.trim() ? onClick : undefined} />
                            </>
                        )
                    }}
                />
