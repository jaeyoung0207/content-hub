 ...
 */
export const StarRatingUi = <T extends FieldValues>({ name, control, selectedStarRating, starRatingErrorMsg }: StarRatingUiProps<T>) => {
    ...
    const createStarState = () => {
        return Array.from(({ length: 5 }), (_, index) => ({ starRating: 0.5 + index }));
    }
    ...
    return (
        ...
        <Controller
            ...
            render={({ field: { value, onChange } }) => {
                ...
                return (
                    <div className="block">
                        <div className="mb-1 flex justify-center" role="radiogroup" aria-label="별점 선택">
                            {
                                // 별점을 루프 돌아가며 표시
                                createStarState().map((items, index) => {
                                    ...
                                    const handleOnMouseLeave = () => {
                                        ...
                                        !isSelected ? onChange(0) : onChange(previousSelected.current);
                                    }
                                    // onClick 시의 처리
                                    ...
                                    return (
                                        <div className="relative w-6 h-6" key={index}>
                                            ...
                                            <div className="absolute z-2 w-1/2 h-full overflow-hidden cursor-pointer"
                                                onMouseEnter={() => !isSelectedStarRating && onChange(halfStarRate)}
                                                onMouseLeave={() => !isSelectedStarRating && handleOnMouseLeave()}
                                                onClick={() => !isSelectedStarRating && handleOnClick(halfStarRate)}
                                            >
                                                {
                                                    // 해당 반 별의 별점 <= 현재 설정된 value값의 경우, 반별 표시
                                                    halfStarRate <= realValue && <BsStarHalf className={starStyle} />
                                                }
                                            </div>

                                            {/* 빈 별 */}
                                            {/* z-index를 통해 우선순위를 정해서 각 별 아이콘이 겹치지 않도록 함 */}
                                            <BsStar className={"absolute z-0 text-2xl " + starStyle} />

                                            {/* 채워진 별 */}
                                            {/* z-index를 통해 우선순위를 정해서 각 별 아이콘이 겹치지 않도록 함 */}
                                            <div className="absolute z-1 w-full h-full cursor-pointer"
                                                onMouseEnter={() => !isSelectedStarRating && onChange(fillStarRate)}
                                                onMouseLeave={() => !isSelectedStarRating && handleOnMouseLeave()}
                                                onClick={() => !isSelectedStarRating && handleOnClick(fillStarRate)}
                                            >
                                                {
                                                    // 해당 채워진 별의 별점 <= 현재 설정된 value값의 경우, 채워진 별 표시
                                                    fillStarRate <= realValue && <BsStarFill className={starStyle} />
                                                }
                                            </div>
                                        </div>
                                    )
