...
export const useDetail = (originalMediaType: string, contentId: string, tabNo: number): useDetailReturnType => {
    ...
    const getDetailApi = async () => {
        ...
        // 원본 미디어 타입이 MOVIE인 경우
            ...
            return (await detailApi.getMovieDetail({ movie_id: contentIdParam })).data;
        }
    ...
    useEffect(() => {
        queryClient.fetchQuery({
            ...
            queryFn: async () => {
                ...
                // 유저 평균 평점 설정
                const convertResponse = response ? response.toFixed(1) : t("info.notExist"); // 소수점 한자리까지 표시(ex: 4 -> 4.0)
