 ...
 * 헤더 컴포넌트의 폼 필드 타입 정의
 */
export type HeaderType = Partial<{ // Partial로 감싸서 객체를 각 필드로 풀어서 정의
    keyword: string,
    aniFlg?: boolean,
    comicsFlg?: boolean,
    movieFlg?: boolean,
...
export const useHeader = (): useHeaderReturnType => {
    ...
    const setAdultFlgMutation = useMutation({
        ...
        mutationFn: async () => await commonApi.setAdultFlg({ adult_flg: adultFlg! })
    });
    ...
    const saveSearchHistory = (selectedKeyword: string) => {
        // 검색 이력 로컬스토리지에서 취득
        let keywords = localStorage.getItem("keywordList") ?? "";
        // 검색어 리스트로 변환
     ...
     * @param selectedKeyword 선택된 자동완성 검색어
     */
    const handleKeywordListOnClick = useCallback((selectedKeyword: string) => {
        // 검색어 입력값 설정
        ...
        // 검색 실행
        navigate(searchUrlQuery({ keyword: selectedKeyword, isAdult: String(adultFlg) }), { replace: true });
    }, [setValue, setAutoCompleteList, setCurrentIndex, saveSearchHistory, navigate, adultFlg]);
    ...
    const handleSearchOnClick = useCallback(() => {
        ...
        if (currentKeyword) {
            ...
            // 검색 실행
            navigate(searchUrlQuery({ keyword: currentKeyword, isAdult: String(adultFlg) }), { replace: true });
        }
    }, [keyword, keywordParam, adultFlg, setCurrentIndex, saveSearchHistory, navigate]);

    /**
     * 검색 이력 삭제 처리
     * @param index 해당 검색 이력 인덱스
     */
    const handleRemoveSearchHistory = useCallback((index: number) => {
        // 검색 이력 로컬스토리지에서 취득
        let keywords = localStorage.getItem("keywordList")!;
        // 검색어 리스트로 변환
    ...
    const handleLogoutOnClick = useCallback(() => {
        // 검색 화면 URL 생성
        const searchUrl = searchUrlQuery({ keyword: keyword!, isAdult: String(adultFlg) });
        // 검색 화면 URL 저장
    ...
    const handleKeywordOnKeyDownEvent = (e: KeyboardEvent) => {
        ...
        if (e.key === ARROW_DOWN_KEY) {
            ...
            const nextIndex = currentIndex + 1 >= autoCompleteList.length ? -1 : currentIndex + 1;
            setCurrentIndex(nextIndex);
            // 다음 인덱스가 -1인 경우, 검색어를 저장된 검색어(검색창에 입력된 검색어)로 설정
            // 다음 인덱스가 -1이 아니면, 검색어를 자동완성 리스트의 해당 인덱스의 검색어로 설정
            const newKeyword = nextIndex === -1
                ? savedKeyword.current
                : autoCompleteList[nextIndex];
        ...
        // 위 화살표 키 입력시
            ...
            const nextIndex = currentIndex === -1 ? autoCompleteList.length - 1
                : currentIndex - 1 < -1 ? -1
                    : currentIndex - 1;
            setCurrentIndex(nextIndex);
            ...
            // 쿠키의 provider가 KAKAO인 경우
                ...
                queryClient.fetchQuery({
                    ...
                    queryFn: async () => {
                        ...
                        const updateResponse = (await loginApi.updateKakaoLoginInfo({ client_id: settings.kakaoClientId })).data;
                        if (updateResponse && updateResponse.userInfo) {
                            // 유저정보 저장
                            setUser(updateResponse.userInfo!);
                            // provider 저장
    ...
    /**
     * 자동완성 debounce 처리
     * 실행 조건: 검색창 클릭/검색어입력 완료시(디바운스 처리된 검색어가 바뀔때마다 실행)
     */
    useEffect(() => {
        // 처음 로드시에만 처리 중지
        if (firstLoadRef.current) {
