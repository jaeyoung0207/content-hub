...
export const headerQueryKeys = {
    ...
    searchKeyword: (keyword: string) => [headerMainKey, "searchKeyword", keyword],
    // 헤더에서 로그인 정보를 업데이트하기 위한 쿼리 키
    login : (provider: string) => [headerMainKey, "login", provider],
    // 헤더에서 성인물 검색 플래그를 설정하기 위한 쿼리 키
    setAdultFlg: () => [headerMainKey, "setAdultFlg"],
    // 헤더에서 성인물 검색 플래그를 해제하기 위한 쿼리 키
    clearAdultFlg: () => [headerMainKey, "clearAdultFlg"],
}