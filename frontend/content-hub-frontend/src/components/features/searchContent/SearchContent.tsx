 ...
 * 각 미디어 검색결과 컴포넌트
 */
const DisplayResults = memo(({ mediaName, results, isViewMore, mediaType, keyword, isAdult }: SearchResultsPropsType) => {
    // i18n 번역 훅
    const { t } = useTranslation();
    // navigate 훅
            ...
            <div className="w-full flex flex-wrap items-start mt-8 ml-5">
                {
                    ...
                    results.map((items, index) => {
                        return (
                            ...
                            >
                                {/* 썸네일 */}
                                <li key={"poster_path" + index} className="flex justify-center items-center">
                                    <img
                                        src={
                                            mediaType === MEDIA_TYPE.COMICS
                                                ? items.backdropPath
